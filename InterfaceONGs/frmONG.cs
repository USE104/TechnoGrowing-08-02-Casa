...
namespace InterfaceONGs
{
    public partial class frmONG : Form
    {
        ...
        {
            MySqlCommand comm = new MySqlCommand();
            comm.CommandText = "update tbONG set Nome = @Nome,Email = @Email, Senha = MD5(@senha) ,Tel = @Tel,CNPJ= @CNPJ,Endereco= @Endereco,Numero = @Numero,Complemento = @Complemento,CEP = @CEP ,Bairro = @Bairro,Cidade = @Cidade, Categoria = @Categoria,Descricao = @Descricao,WebSite =@WebSite, RedeSocial =@RedeSocial, Url = @Url";
            comm.CommandType = CommandType.Text;
            comm.Connection = Conexao.obterConexao();
        ...
        {
            MySqlCommand comm = new MySqlCommand();
            comm.CommandText = "select * from tbONG  where  codONG like '%" + txtNomeONG+ "%'";
            comm.CommandType = CommandType.Text;
            comm.Connection = Conexao.obterConexao();
            ...
            while (dr.Read())
            {
                ...
                cbbRedeSocial.Text = dr.GetString(15);
                txtUrl.Text = dr.GetString(16);
            }
            Conexao.fecharConexao();
